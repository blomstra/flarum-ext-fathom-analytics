...
flarum_forum_app__WEBPACK_IMPORTED_MODULE_0___default().initializers.add('blomstra/fathom-analytics', function () {
  setTimeout(function () {
    flarum_forum_app__WEBPACK_IMPORTED_MODULE_0___default().fathomEventsRepository.propogateServerEventData();
    flarum_forum_app__WEBPACK_IMPORTED_MODULE_0___default().fathomEventsRepository.getEnabledEvents().forEach(function (event) {
      var _event$requiresExtens;

      if ((_event$requiresExtens = event.requiresExtensions) != null && _event$requiresExtens.some(function (ext) {
        return !(ext in flarum.extensions);
      })) {
